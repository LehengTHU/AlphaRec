...
def fix_seeds(seed=101):
    ...
    os.environ['PYTHONHASHSEED'] = str(
        seed)  # In order to disable hash randomization and make the experiment reproducible.
    np.random.seed(seed)
...
def visualize_and_save_log(file_dir, dataset_name, show=False):
    ...
    if (dataset_name == "tencent_synthetic"):
        ...
        # Define table.
        result = pd.DataFrame(
            {'epochs': epochs, 'valid_recall': valid_recall, 'test_recall': test_recall, 'valid_ndcg': valid_ndcg,
             'test_ndcg': test_ndcg})
